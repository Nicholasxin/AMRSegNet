...
class SE_block(nn.Module):
    ...
    def __init__(self, planes, stride=1, downsample=None):
        ...
        if planes == 50:
            self.globalAvgPool = nn.AvgPool3d((planes//2, 256,256), stride=(planes//2, 1,1))
        elif planes == 100:
            self.globalAvgPool = nn.AvgPool3d((planes//2, 128,128), stride=(planes//2, 1,1))
        elif planes == 200:
            self.globalAvgPool = nn.AvgPool3d((planes//2, 64,64), stride=(planes//2, 1,1))
        elif planes == 400:
            self.globalAvgPool = nn.AvgPool3d((planes//2, 32,32), stride=(planes//2, 1,1))
        elif planes == 800:
            self.globalAvgPool = nn.AvgPool3d((planes//2, 16,16), stride=(planes//2, 1,1))
    ...
    def forward(self, x):
        ...
        original_x = x
        #pdb.set_trace()
        # x_T2 = x[:, :self.planes//2, :, :]
        # x_DWI = x[:, self.planes//2:, :, :]
        # out1 = self.globalAvgPool(x_T2)
        # out2 = self.globalAvgPool(x_DWI)
        ...
        out25 = torch.squeeze(out[:,:out.size(1)//2,:,:]).item()
        out50 = torch.squeeze(out[:,out.size(1)//2:,:,:]).item()
        # pdb.set_trace()
        # for _ in range(self.planes//2 -1):
        #     out25 = torch.cat((out25,out[:,:out.size(1)//2,:,:]),dim=1)
        #     out50 = torch.cat((out50,out[:,out.size(1)//2:,:,:]),dim=1)
        original_x_25 = out25 * original_x[:, :original_x.size(1)//2, :, :]
        original_x_50 = out50 * original_x[:, original_x.size(1)//2:, :, :]
        out = torch.cat((original_x_25, original_x_50), dim=1)
        # out = out * original_x
...
class up_conv_block(nn.Module):
    def __init__(self, ch_in, ch_out, depth, relu=False):
        ...
        self.sequentialconv = _make_nConv(ch_out, depth, relu)
        self.SE_block1 = SE_block(planes=ch_out//2)
